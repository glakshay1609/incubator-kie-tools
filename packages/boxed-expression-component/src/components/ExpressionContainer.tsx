...
export interface ExpressionContainerProps {
  ...
  expressionDiffsById?: Map<string, ChangeMap> | undefined;
  listItemDiffStatus?: string|undefined;
}
...
export const ExpressionContainer: React.FunctionComponent<ExpressionContainerProps> = ({
  ...
  // Determine diff status - but don't color if this is a row-level change
  // Row-level changes are handled by the row index column highlighting
  const exprId = expression?.["@_id"];
  const exprDiffs = exprId ? expressionDiffsById?.get(exprId) : undefined;
  let diffStatus: "added" | "deleted" | "updated" | undefined;

  if (exprDiffs) {
    for (const changes of exprDiffs.values()) {
      for (const c of changes) {
        // Only color if this is NOT a row-level add/delete
        // Row-level changes have path === exprId (the item itself being added/deleted)
        // Expression-level changes have path like "exprId.text.__$$text" or "exprId.__$$element"
        const isRowLevelChange = c.path === exprId || c.path === `${exprId}`;
  ...
  // Don't apply background color if listItemDiffStatus is present (row-level change)
  // or if this is a nested expression within a list item
  const shouldShowBackground = !listItemDiffStatus && diffStatus;

  return (
    <div ref={containerRef} className={`expression-container-box }`}
      style={{
        backgroundColor: shouldShowBackground
          ? diffStatus === "added"
            ...
            ? "#F8D7DA"   // red
            : diffStatus === "updated"
            ? "rgba(255, 193, 7, 0.2)" // amber/yellow for updates
            : "transparent"
          : "transparent",
      }}
